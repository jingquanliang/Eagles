...
var srcId=(window.location.pathname).split("/")[1]; //自己的id
srcId=srcId.replace(/\//g,'');

var srcName=(window.location.pathname).split("/")[2]; //自己的用户名
srcName=srcName.replace(/\//g,'');

var desId=0;//0是系统管理员，点击相应的用户时候会改变
...
function generateUser(data)
{
        ...
        $("#imjs-main-contact-list").append(firstEle);
...
$(function() {
    ...
    chatsock = new ReconnectingWebSocket(ws_scheme + '://' + window.location.host + "/chat" + "/"+srcId);
    ...
    chatsock.onmessage = function(message) {
        ...
        if(srcId=="0") //这个srcId是全局变量，不要混淆
        {//说明本页面是客服，也就是后台,需要根据desid生成相应的联系人
            //首先判断，此用户是否已经存在了
            //var ss=eles.length;
...
$(function() {
        ...
        $(".ePanelarea > li > a").on("click",function(){
            ...
            showContent(srcId,imgele,getNowFormatDate()); //在自己的页面展示
            ...
            //
            //var contentDiv=$("<div class='webim-body-comtent-msg-right imjs-msg-content'><s>&nbsp;</s></div>");
            //contentDiv.prepend(imgele);
            //
            //firstEle.append(contentDiv);
            //
            //var contentTime=$("<span class='webim-times imjs-msg-time'></span>");
            //contentTime.text(getNowFormatDate());
            //
            //firstEle.append(contentTime);
            //
            //chatList.append(firstEle);
